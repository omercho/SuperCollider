...
IFMopho {
	...
	*loadSC{
		...
		~sin1Mopho=0.1;
		~sin2Mopho=0.9;
		~rootFreqMopho=~c5; // 261=C4|523=C5
		IFMopho.synthDef(2);
		~defaultBufMopho.free;~defaultBufMopho = Buffer.read(Server.default, "/Applications/SuperCollider/sounds/_IFSC_Sounds/dop01.aif");
		~bufnumMopho = PatternProxy( Pseq([~defaultBufMopho], inf));
		~bufnumMophoP = Pseq([~bufnumMopho], inf).asStream;
	...
	*proxy {
		...
		~lfo2Mopho = PatternProxy( Pseq([10], inf));
	...
	*osc{
		...
		~actMophoBut = OSCFunc({
			...
			if ( msg[1]==1, {
				~actMopho.source=1;
				~apc40.noteOn(~apcMnCh, ~actButA7, 127); //Trk4_But 1
			},{
				~actMopho.source=0;
				~apc40.noteOff(~apcMnCh, ~actButA7, 127); //Trk4_But 1
			});
		},
		'/activMopho'
		);
		...
		~time2MophoBut= OSCFunc({
			...
			~countTime2Mopho.switch(
				0,{},
				1, {
					~apc40.noteOn(~apcMnCh, ~actButB7, 1); //Trk4_But 2
				...
				2,{
					...
					~apc40.noteOn(~apcMnCh, ~actButB7, 0); //Trk4_But 2
					~countTime2Mopho=0;
				}
			);
		},
		...
		~susMophoFader= OSCFunc({
			...
			if ( ~volcaBoolean==1, {
				...
				~vMopho.control(~chMopho, ~slideTime, vel);
				~susLevMopho=val;
			},{
				~tOSCAdrr.sendMsg('susMopho', msg[1]);
				~susLevMopho=val;
		...
		~decMophoFader= OSCFunc({
			...
			if ( ~volcaBoolean==1, {
				...
				~decMopho=val;
				~relMopho=val*0.7;
				~vMopho.control(~chMopho, ~egDec, vel);
			},{
				~tOSCAdrr.sendMsg('decMopho', val);
				~decMopho=val;
				~relMopho=val*0.7;
		...
		~xy1Mopho= OSCFunc({
			...
			vel=msg[1]*127;
			if ( ~volcaBoolean==1, {
				~vMopho.control(~chMopho, ~vcoPitch2, msg[2]*127);
				~vMopho.control(~chMopho, ~vcoPitch3, msg[1]*127);
